...
import { generateLastNames, type LastNameGeneratorParams } from "@/lib/generators/lastname"
...
export function LastNameGeneratorClient() {
  ...
  const [results, setResults] = useState<ReturnType<typeof generateLastNames>>([])
  ...
  const updateURL = useCallback((currentSeed: string, params: LastNameGeneratorParams) => {
  ...
  const generate = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      const params: LastNameGeneratorParams = {
        count,
        unique,
        seed: currentSeed,
      }
      
      const newResults = generateLastNames(params)
      ...
      saveToHistory({
        ...
        seed: currentSeed,
        params,
        outputPreview: newResults.slice(0, 3).map(r => r.lastName).join(", "),
        timestamp: Date.now(),
      })
  ...
  }, [count, unique, getOrCreateSeed, tool.slug, updateURL])
  ...
  const handleCopyAll = () => {
    return results.map(r => r.lastName).join("\n")
  }

  const handleDownloadCSV = () => {
    const csv = results.map(r => r.lastName).join('\n')
    const blob = new Blob([csv], { type: 'text/csv' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = 'last-names.csv'
    a.click()
  ...
  const shareUrl = () => {
    ...
    return `${window.location.origin}/last-name-generator?${params.toString()}`
  }
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <div className="space-y-6">
            ...
            <div className="flex items-center space-x-2">
              ...
              />
              <Label htmlFor="unique">Unique last names (no duplicates)</Label>
            </div>
            ...
            >
              {isGenerating ? "Generating..." : "Generate Last Names"}
            </Button>
          </div>
          ...
          <div className="space-y-4">
            {isGenerating ? (
              ...
              <div className="border rounded-lg p-12 text-center text-muted-foreground">
                Click "Generate Last Names" to start
              </div>
            )}
          </div>
        </div>
